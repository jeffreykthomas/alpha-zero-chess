...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory=None, goes_first=0):
    ...
    for e in range(EPISODES):
        ...
        logger.info('====================')
        logger.info('POINTS TOTAL: ' + str(points))
        ...
        while done == 0:
            ...
            moves_sorted = pi.argsort()
            moves_ranked = moves_sorted[(pi > 0)[moves_sorted]]
            for r in range(len(moves_ranked)):
                logger.info('%s probability: %f', state.decodeMove(moves_ranked[r]),
                            pi[moves_ranked[r]])
