...
def encode_board(gameState):
    board_state = gameState.board.current_board
    ...
    if gameState.board.K_move_count != 0:
        encoded[:, :, 13] = 1  # cannot castle queenside for white
        encoded[:, :, 14] = 1  # cannot castle kingside for white
    if gameState.board.K_move_count == 0 and gameState.board.R1_move_count != 0:
        encoded[:, :, 13] = 1
    if gameState.board.K_move_count == 0 and gameState.board.R2_move_count != 0:
        encoded[:, :, 14] = 1
    if gameState.board.k_move_count != 0:
    ...
    if gameState.board.k_move_count == 0 and gameState.board.r2_move_count != 0:
        encoded[:, :, 16] = 1
    encoded[:, :, 17] = gameState.board.move_count
    encoded[:, :, 18] = gameState.draw_counter
    encoded[:, :, 19] = gameState.repetitions_b
    encoded[:, :, 20] = gameState.no_progress_count
    encoded[:, :, 21] = gameState.board.en_passant
    return encoded


def decode_board(gameState, encoded):
    ...
    gameState.board = decoded
    if encoded[0, 0, 12] == 1:
        gameState.playerTurn = 1
    if encoded[0, 0, 13] == 1:
        gameState.R1_move_count = 1
    if encoded[0, 0, 14] == 1:
        gameState.R2_move_count = 1
    if encoded[0, 0, 15] == 1:
        gameState.r1_move_count = 1
    if encoded[0, 0, 16] == 1:
        gameState.r2_move_count = 1
    gameState.move_count = encoded[0, 0, 17]
    gameState.draw_counter = encoded[0, 0, 18]
    gameState.repetitions_b = encoded[0, 0, 19]
    gameState.no_progress_count = encoded[0, 0, 20]
    gameState.en_passant = encoded[0, 0, 21]
    return board


def encode_action(gameState, initial_pos, final_pos, underpromote=None):
    ...
    x, y = final_pos;
    dx, dy = x - i, y - j
    piece = gameState.board.current_board[i, j]
...
def decode_action(board, playerTurn, encoded):
